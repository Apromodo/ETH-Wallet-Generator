...
const argv = Yargs(process.argv.slice(2))
  ...
  .alias("s", "suffix")
  .string("s")
  .describe("s", "input hex string for suffix")
  .alias("c", "checksum")
  .boolean("c")
...
if (cluster.isMaster) {
  const args = {
    ...
    suffix: argv.suffix ? argv.suffix : "",
  ...
  if (!VanityEth.isValidHex(args.input) || (args.suffix && !VanityEth.isValidHex(args.suffix))) {
    console.error("Input and suffix must be valid hexadecimal strings");
    process.exit(1);
  }
  let logStream;
  if (args.log) {
  ...
  while (true) {
    if (process.send) {
      const account = VanityEth.getVanityWallet(
        worker_env.input,
      ...
      if (account && account.address.endsWith(worker_env.suffix)) {
